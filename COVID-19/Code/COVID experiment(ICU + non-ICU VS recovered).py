...
label_int = []
for i in range(0, ICU_patients_datas.shape[0]):
    label_int.append(0) # "not recovered" = 0
for i in range(0, non_ICU_patients_datas.shape[0]):
    label_int.append(0) # "not recovered" = 0
for i in range(0, recovered_patients_datas.shape[0]):
    label_int.append(1) # "recovered" = 1
...
for i in range(0, len(affect_coef)):
    if (affect_coef[i][1] != 0):
        name_svc.append(affect_coef[i][0] + "(" + marker_cell_dict[affect_coef[i][0]] + ": " + CD_marker_to_cell[marker_cell_dict[affect_coef[i][0]]] + ")")
        coef.append(affect_coef[i][1])
    else:
        name_svc.append(affect_coef[i][0] + "(" + marker_cell_dict[affect_coef[i][0]] + ": " + CD_marker_to_cell[marker_cell_dict[affect_coef[i][0]]] + ")" + "(0 coef)")
        coef.append(affect_coef[i][1])
...
fig.set_size_inches(18.5, 10.5)
axs.scatter(x_label, coef, color = color)
plt.title("Ranking of The Affect Coefficients(Absolute Value) for LinearSVC", fontsize = 20)
plt.ylabel('Coefficient', size = 20)
plt.xlabel('Ranking', size = 20)
plt.xticks(fontsize = 20)
plt.yticks(fontsize = 20)
...
# List detailed numbers and cell names
print("The ranking of the affect coefficients(absolute value) for LinearSVC is:")
zero_position = 0
for i in range(0, len(affect_coef)):
    if (affect_coef[i][1] != 0):
        print(str(i + 1) + ". " + str(affect_coef[i][0]) + "(" + marker_cell_dict[affect_coef[i][0]] + ": " + CD_marker_to_cell[marker_cell_dict[affect_coef[i][0]]] + ")" + ": " + str(affect_coef[i][1]))
...
fig = go.Figure(data=[go.Table(
    header=dict(values=['Rank', 'LASSO', 'LinearSVC'],
                fill_color='paleturquoise',
                align='center'),
    cells=dict(values=[x_label, name, name_svc],
               fill_color='lavender',
               align='center'))
])
...
# #### Calculate the correlation coefficient and the p value for the two rankings

# #### The p-value roughly indicates the probability of an uncorrelated system producing datasets that have a Pearson correlation at least as extreme as the one computed from these datasets.

# ![Spearman%20VS%20Pearson-2.png](attachment:Spearman%20VS%20Pearson-2.png)

# In[12]:


# Spearman rank-order correlation
spearman_result = spearmanr(lasso.coef_, svc.coef_[0])

# Pearson correlation
person_result = pearsonr(lasso.coef_, svc.coef_[0])
print("If use all 59 coefficients:")
print("According to Spearman, the correlation coefficient is: " + str(spearman_result.correlation) + " The p-value is: " + str(spearman_result.pvalue))
print("According to Pearson, the correlation coefficient is: " + str(person_result[0]) + " The p-value is: " + str(person_result[1]))
...
for i in range(0, len(lasso.coef_)):
    if lasso.coef_[i] == 0 and svc.coef_[0][i] == 0:
        continue
    else:
        new_lasso_coef.append(lasso.coef_[i])
        new_svc_coef.append(svc.coef_[0][i])

# Spearman rank-order correlation
spearman_result = spearmanr(new_lasso_coef, new_svc_coef)

# Pearson correlation
person_result = pearsonr(new_lasso_coef, new_svc_coef)
print("If exclude both-zero coefficients, we will have " + str(len(new_lasso_coef)) + " coefficients left.")
print("According to Spearman, the correlation coefficient is: " + str(spearman_result.correlation) + " The p-value is: " + str(spearman_result.pvalue))
print("According to Pearson, the correlation coefficient is: " + str(person_result[0]) + " The p-value is: " + str(person_result[1]))
...
result = []
result.append(["Rank", "LASSO", "LinearSVC"])
for i in range(0, len(x_label)):
    step_result = []
    step_result.append(x_label[i])
    step_result.append(name[i])
    step_result.append(name_svc[i])
    result.append(step_result) 
np.savetxt('D:\\COMP 495\\COVID-19\\COVID-19 project_Brodinlab\\Influence factor comparison(not recovered VS recovered).csv', result, delimiter=",", newline = "\n", fmt="%s")
...
for i in range(0, ICU_patients_datas_validation.shape[0]):
    ...
    print("not recovered patient: is determined to be a " + result[0] + " patient")
    if (result[0] == 'not recovered'):
